...
class Kpi_detaillog(db.Model):
    ...
    def __repr__(self):
        return "<Kpi_detaillog %r>" % self.id
...
class Kpi_personlog(db.Model):
    ...
    def __repr__(self):
        return "<Kpi_personlog %r>" % self.id


# KPI详情操作日志
class tbl_table_a_result(db.Model):
    __tablename__ = "tbl_table_a_result"
    __bind_key__ = 'tools_db'
    result_id = db.Column(db.Integer, primary_key=True)
    line_num = db.Column(db.Integer)
    station_name = db.Column(db.String(1000))
    box_sum = db.Column(db.Float)
    who_first = db.Column(db.String(1000))
    drivers = db.Column(db.String(1000))
    lines = db.Column(db.String(1000))
    remark = db.Column(db.String(1000))
    order_date = db.Column(db.DateTime, index=True, default=datetime.now)
    station_name1 = db.Column(db.String(1000))
    station_name2 = db.Column(db.String(1000))

    def __repr__(self):
        return "<tbl_table_a %r>" % self.result_id


# 要货量头表
class tbl_trans_things_num_head(db.Model):
    __tablename__ = "tbl_trans_things_num_head"
    __bind_key__ = "tools_db"
    things_head_id = db.Column(db.Integer, primary_key=True)
    station_num = db.Column(db.DECIMAL)
    box_num_sum = db.Column(db.DECIMAL)
    order_date = db.Column(db.DateTime, index=True, default=datetime.now)
    import_by = db.Column(db.String(500))

    def __repr__(self):
        return "<tbl_trans_things_num_head %r>" % self.things_head_id
...
class tbl_trans_things_num_temp(db.Model):
    ...
    def __repr__(self):
        return "<tbl_trans_things_num_temp %r>" % self.things_id


# 要货量
class tbl_trans_things_num_template(db.Model):
    __tablename__ = "tbl_trans_things_num_template"
    __bind_key__ = "tools_db"
    things_id = db.Column(db.Integer, primary_key=True)
    station_name = db.Column(db.String(1000))
    per_box_num = db.Column(db.Float)
...
# # 要货量
# class tbl_trans_things_num(db.Model):
#     __tablename__ = "tbl_trans_things_num"
#     __bind_key__ = "tools_db"
#     station_name = db.Column(db.String(1000))
#     per_box_num = db.Column(db.Float)
#     box_num = db.Column(db.Float)
...
#
#     def __repr__(self):
#         return "<tbl_trans_things_num %r>" % self.station_name
